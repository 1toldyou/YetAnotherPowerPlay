...
public class HoldPositionCommand extends CommandBase {
    ...
    public HoldPositionCommand(Drivetrain drivetrain, Localizer localizer, Pose targetPose, double ms) {
    ...
    public void execute() {
        ...
        boolean correct = ((Math.hypot(error.x, error.y) < ALLOWED_TRANSLATIONAL_ERROR * 4) && (Math.abs(error.heading) < ALLOWED_HEADING_ERROR * 4));

        if (!correct) drivetrain.set(goToPosition(localizer.getPos(), targetPose));
    }
